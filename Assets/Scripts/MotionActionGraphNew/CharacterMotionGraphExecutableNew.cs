...
using CoolAnimation.InstantNodes;
using UnityEngine;

namespace CoolAnimation
{
    public class CharacterMotionGraphExecutableNew
    {
        private List<BaseMotionNodeExecutableNew> _allNodes = new List<BaseMotionNodeExecutableNew>();

        public List<BaseMotionNodeExecutableNew> AllNodes => _allNodes;

        private HashSet<MotionContinuousActionNodeExecutableNew> _activeContinuousNodes =
            new HashSet<MotionContinuousActionNodeExecutableNew>();

        private HashSet<BaseMotionNodeExecutableNew> _nodesWaitingForExecution = new HashSet<BaseMotionNodeExecutableNew>();

        private HashSet<BaseMotionNodeExecutableNew> _justFinishedNodes = new HashSet<BaseMotionNodeExecutableNew>();

        private List<MotionContinuousActionNodeExecutableNew> _continuousMotionToRemove =
            new List<MotionContinuousActionNodeExecutableNew>();

        private StartMotionNodeExecutableNew _startingNode;

        public StartMotionNodeExecutableNew StartingNode {
            get
            {
                ...
                else
                {
                    ...
                    {
                        if (nodeExecutable is StartMotionNodeExecutableNew startNode)
                        {
        ...
        public void BuildFromTemplate(CharacterMotionGraphNew graphTemplate)
        {
            Dictionary<BaseMotionNodeNew, BaseMotionNodeExecutableNew> templateAssociation =
                new Dictionary<BaseMotionNodeNew, BaseMotionNodeExecutableNew>();

            foreach (var node in graphTemplate.nodes)
            {
                if (node is BaseMotionNodeNew motionNode)
                {
        ...
        {
            var nodesToExecute = new List<BaseMotionNodeExecutableNew>();
        ...
        private void ExecuteNode(BaseMotionNodeExecutableNew node, CharacterMotionGraphExecutionContext context)
        {
            ...
            else if(node is MotionContinuousActionNodeExecutableNew continuousNode)
            {
